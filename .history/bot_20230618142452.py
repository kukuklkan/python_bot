...
def calldack(call):
    ...
    if call.message:
        ...
        if call.data == 'taryf_for_hand_made':
            ...
            with open ('filetered.json', 'r') as filtered:
                filtered_gigabytes = json.load(filetered)
                filtered_minutes =  [tariff for tariff in tarrifs if tariff[list(tariff.keys())[0]]['minutes'] >= 500]
